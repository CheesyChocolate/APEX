  ...
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \begin{block}{Step 1 — Lipinski Rule of Five}
        {\small Drug-likeness gate on predicted actives:
        \begin{itemize}\setlength{\itemsep}{1pt}
          \item MW $\leq$ 500\,Da
          \item LogP $\leq$ 5
          \item HBD $\leq$ 5,\quad HBA $\leq$ 10
          \item TPSA $\leq$ 140\,\AA$^2$
        \end{itemize}
        Falls back to all actives if none pass.}
      \end{block}
    \end{column}
    \begin{column}{0.47\textwidth}
      \begin{block}{Step 2 — Max-Min Diversity}
        {\small Greedy farthest-point on Tanimoto distance:
        \begin{enumerate}\setlength{\itemsep}{1pt}
          \item Seed: highest-$p$ drug-like compound
          \item Add compound most dissimilar to all selected
          \item Stop at $N = 20$
        \end{enumerate}
        $d_T(A,B) = 1 - \frac{|A \cap B|}{|A \cup B|}$}
      \end{block}
    \end{column}
  \end{columns}
  \vspace{-0.1cm}
  \begin{exampleblock}{Why Diversity?}
    \small Similar scaffolds share binding modes — diversity maximises
    binding-pocket exploration and avoids redundant docking runs.
  \end{exampleblock}
\end{frame}
